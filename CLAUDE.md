...
- **Cowork's scheduled prompts live in `cowork/tasks.toml`, not in code.** A Cowork scheduled task is
  ...
  `--check` so the two cannot drift. **But that check runs when `tasks.toml` is loaded, not when a tool
  is called.** The MCP endpoint hands every tool to anyone holding `MCP_TOKEN` — there is no
  per-task or per-caller gating — so a read-only Cowork session really is holding `pending_actions`
  and `report_action`, and the browser it is holding is signed in to ESPN. A prompt claiming "there
  is no tool here that can change the roster" was therefore falsifiable with one `list_tools`, and
  an instruction a model can catch lying to it earns none of the trust the rest of it needs. The
  read-only prompts now **instruct** rather than assert: they name the acting tools, say the session
  may well be holding them, and say not to call them.
  `tests/unit/test_readonly_prompt_truth.py` fails on any read-only prompt that goes back to
  claiming the tools are absent, and on any that drops the instruction while doing so. Real
  enforcement needs a second endpoint with its own token exposing only the read tools; until that
  exists, the prompt is the whole boundary and it has to be honest to hold.
