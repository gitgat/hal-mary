...
- `loop.py`: `DraftLoop(conn, settings, client, runner, bus)` with `warm()`, `async run_once()`, `async run_forever()`/`stop()`, and `record_manual_pick`. Warms `client.player_name_map()` before the first poll. Every `config.draft.poll_seconds` (5) it syncs, then applies `pending_picks(conn)` — every recorded pick the board does not yet agree with, deduplicated — marks the board, files unmatched picks in the `unmatched_picks` table, broadcasts `board_updated`, and runs the advisor when `picks_until_mine <= config.draft.advise_within_picks` **and it has not already advised for that pick of hers**. Gates on `my_upcoming_picks` being empty, the only end-of-draft signal there is. Nothing escapes: an ESPN failure is logged and the loop lives. Runs on its own thread with its own connection; the event bus carries its events to the web loop.
